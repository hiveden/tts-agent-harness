...
test("V1 — 核心痛点演示", async ({ page, context }) => {
  ...
  const chunkRow = page.locator(`text=${CHUNK_ID}`).first();
  await chunkRow.scrollIntoViewIfNeeded();
  await page.waitForTimeout(1500);

  // S02: 播放 take#1 — 原始版本，发音有问题
  mark("试听 take#1 — "第Ⅱ章"的 Ⅱ 被读成了"一"");
  const takeSection = chunkRow.locator("..").locator("..").locator(".."); // navigate to row container
  // Click the first take's play button
  const take1Play = page.locator("text=#1").first().locator("..").locator("button:has-text('▶')");
  if (await take1Play.isVisible()) {
  ...
  const take4Play = page.locator("text=#4").first().locator("..").locator("button:has-text('▶')");
  if (await take4Play.isVisible()) {
  ...
  // Click edit button on this chunk
  const editBtn = chunkRow.locator("..").locator("..").locator("button:has-text('✎')");
