...
| 痛点 | 直接原因 | 根因 |
|------|---------|------|
| 导出卡全站 | `subprocess.run` 阻塞事件循环 | async 端点中混入同步调用 |
| 批量导出失败 | 导出是同步请求，串行排队 | 重活没有交给任务队列 |
| Pipeline 重复运行 | check-then-act 非原子 | 缺少 DB 级并发控制 |
| SSE 断连风暴 | 事件循环被阻塞无法发心跳 | 同上：同步阻塞 |
| 连接池耗尽 | 默认配置偏小 | 未按生产负载调参 |
